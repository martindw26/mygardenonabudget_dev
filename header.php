...
if (function_exists('have_rows')):
    ...
    if (have_rows('left_page_skin_ads')): 
        ...
        while (have_rows('left_page_skin_ads')): the_row();
            $image = get_sub_field('left_page_skin');   // Get the ad image URL
            $url = get_sub_field('left_page_skin_url'); // Get the ad URL

            // Check if both image and URL are not empty
            if ($image && $url) {
                $ads[] = [
                    'image' => $image,
                    'url' => $url
        ...
        if (!empty($ads)) {
            ...
            // Display message if no ads are available
            echo '<!-- No ads available in the repeater field -->';
        }
    else:
        // Display message if the repeater field has no data
        echo '<!-- Repeater field "left_page_skin_ads" has no rows -->';
    endif;

else:
    // Display message if ACF is not activated
