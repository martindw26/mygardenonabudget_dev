...
<div class="refine-search mb-4">
    <form id="refine-search-form">
        ...
        <div class="filters-container">
            ...
            <div class="filter-container">
                <label for="filter-material-type" class="form-label">Material Type</label>
                <select id="filter-material-type" class="form-select" multiple aria-label="Filter by Material Type">
                    <?php
                    $materialTypes = array_unique(array_column($products, 'material_type'));
                    foreach ($materialTypes as $type) : ?>
                        <option value="<?php echo esc_attr($type); ?>"
                            <?php echo isset($urlParams['material_type']) && in_array($type, explode(',', $urlParams['material_type'])) ? 'selected' : ''; ?>>
                ...
                foreach ($products as $product) {
                    if ($product['position'] == $position) {
                        ...
                        <tr data-name="<?php echo esc_attr($product['name']); ?>" data-currency="<?php echo esc_attr($product['currency']); ?>" data-price="<?php echo esc_attr($product['price']); ?>" data-plant-type="<?php echo esc_attr($product['plant_type']); ?>" data-soil-type="<?php echo esc_attr($product['soil_type']); ?>" data-planting-position="<?php echo esc_attr($product['planting_position']); ?>">
                            ...
                            <td><?php echo esc_html($product['material_type']); ?></td> <!-- Display plant type -->
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const plantTypeSelect = form.querySelector('#filter-material-type'); // Add plant type select
    ...
    function getQueryParams() {
        ...
        if (selectedPlantTypes.length > 0) {
            params.append('material_type', selectedPlantTypes.join(','));
        }
    ...
    function filterTable() {
        ...
        rowsArray.forEach(row => {
            ...
            const plantType = row.getAttribute('data-material-type');
